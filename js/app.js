...
class PaperReviewerApp {
    ...
    extractPublicationYearsFromData(data) {
        const wos = data?.wos_data || {};
        const raw = wos.publication_year ?? wos.publicationYear ?? wos.pub_year ?? wos.year;
        return this.normalizePublicationYears(raw);
    }

    async getPublicationYearStatsForCurrentView({ view = '', log = true } = {}) {
        ...
        await Promise.all(bases.map(async (base) => {
            ...
            try {
                const data = await this.readProjectFile(path);
                const years = this.extractPublicationYearsFromData(data);
                if (!years.length) {
                    missingYear += 1;
                    return;
                }
                ...
                console.warn('Publication year stats read failed for', base, err);
                readErrors += 1;
            }
        }));
        ...
        const sortedYears = Object.keys(byYear)
            ...
            .sort((a, b) => a - b)
            .map(year => ({ year, count: byYear[String(year)] }));
        ...
        if (log) {
            console.log(`Publication year stats for view "${targetView}"`);
            if (sortedYears.length) {
                console.table(sortedYears);
            } else {
                console.log('No publication_year found.');
            }
            console.log('Summary:', { totalFiles: bases.length, missingYear, readErrors });
        }
